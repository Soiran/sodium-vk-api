...
class Bot {
    ...
    localMessage(response, ...handlers) {
        this.eventHandlers.forEach(h => h(this._externalEventParsers['message_new'](response, ...handlers)))
    }

    start(callback) {
        if (!this.enabled) {
            this.enabled = true
            this._controller.event('message_new', r => {
                this._eventHandlers['message_new'].forEach(h => h(this._wrapResponse(r)))
            })
            this._controller.event('message_event', r => {
                this._eventHandlers['message_event'].forEach(h => h(this._wrapResponse(r)))
            })
            this._controller.startPolling(callback)
        }
    }

    event(event, callback=_=>{}) {
        if (!this._eventHandlers[event]) {
            this._eventHandlers[event] = [ r => callback(this._wrapResponse(r)) ]
        } else {
            this._eventHandlers[event].push(r => callback(this._wrapResponse(r)))
        }
    }
    ...
    addStages(...stages) {
        stages.forEach(s => {
            if (!(c instanceof Stage)) throw 'Invalid stage object.'
            this.stages.push(s)
